...
video_dir="Y:\\import backups\\手动摇杆驱动程序\\91porn"
##用来存储video目录的mtime信息，判断目录是否改动过,以及上次扫描到的video目录内的文件信息
json_path="C:\\Users\\yuri\\Desktop\\temp.temp"
...
try:
	...
	print('*'*50)
	##结果文件进行序列化
	with open('C:\\Users\\yuri\\Desktop\\temp.temp','w') as fileh:
		json.dump(res_dict,fileh)
else:
	##上次结果文件还在，直接load
	print("结果文件还在，加载mtime信息~")
	with open('C:\\Users\\yuri\\Desktop\\temp.temp','r') as fileh:
...
try:
	...
	print("json文件中记录video目录mtime为："+time.ctime(res_dict['last_mtime_value']))
